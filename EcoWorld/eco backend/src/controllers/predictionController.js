...
const getPredictions = asyncHandler(async (req, res) => {
  ...
  const predictions = zones.map((z) => {
    ...
    // A day-by-day view derived from the two real weekly totals (linear
    // interpolation), so the chart has more than two points without
    // inventing any new underlying data.
    const series = DAYS.map((day, i) => ({
      day,
      current: Math.round((current / 7) * (i + 1)),
      predicted: Math.round((predicted / 7) * (i + 1)),
    }));
