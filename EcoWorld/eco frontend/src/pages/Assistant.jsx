...
export default function Assistant() {
  ...
  async function send(question) {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: 'ai', text: res.answer, mock: res.source === 'mock' }]);
    } catch (err) {
      setMessages((m) => [...m, { role: 'ai', text: `Error: ${err.message || 'Failed to reach the assistant'}`, mock: true }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <AppLayout title="AI Database Assistant" subtitle="Ask questions about your city's waste data">
      <PageHead title="AI Database Assistant" subtitle="Ask about zones, bins, or categories in plain language. Grounded in live data across every zone (admin/staff only)." />
            ...
            >
              {m.role === 'ai' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--ink-muted)' }}>
                  <Sparkles size={12} color="var(--signal-mint)" /> EcoMind AI {m.mock && '(mock)'}
                </div>
              )}
              <div
                style={{
